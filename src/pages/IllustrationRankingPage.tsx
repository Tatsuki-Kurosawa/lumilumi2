...
const IllustrationRankingPage: React.FC = () => {
  ...
  const weeklyRanking = [
    {
      ...
      likes: 1845,
      views: 12520,
      totalPoints: 14365, // PV + いいね
      tags: ['イラスト', 'ファンタジー', 'キャラクター'],
      rank: 1,
      rankChange: 0, // 前週比
    },
    {
  ...
  const popularTags = [
    { name: 'ファンタジー', count: 52, works: weeklyRanking.filter(w => w.tags.includes('ファンタジー')) },
    { name: 'キャラクター', count: 48, works: weeklyRanking.filter(w => w.tags.includes('キャラクター')) },
    { name: '風景', count: 35, works: weeklyRanking.filter(w => w.tags.includes('風景')) },
    { name: 'アニメ', count: 32, works: weeklyRanking.filter(w => w.tags.includes('アニメ')) },
    { name: '水彩', count: 28, works: weeklyRanking.filter(w => w.tags.includes('水彩')) },
    { name: 'SF', count: 22, works: weeklyRanking.filter(w => w.tags.includes('SF')) },
  ];
