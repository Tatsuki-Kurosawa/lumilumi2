...
import { Mail, BookOpen, HelpCircle, Share2, X, Copy, Check } from 'lucide-react';
...
const Footer: React.FC = () => {
  ...
  const shareText = `リンクをクリックして${userCount.toLocaleString()}人の学生クリエイターと一緒に創作活動に参加しよう！\n${shareUrl}`;
      ...
      {showShareModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-medium max-w-md w-full p-6 sm:p-8 relative">
            ...
            {/* タイトル */}
            <h2 className="text-2xl font-bold text-text-primary mb-6">サイトを共有</h2>

            {/* 共有テキスト */}
            <div className="mb-6">
              <p className="text-text-secondary text-base leading-relaxed mb-4">
                {loading ? (
                  <span className="text-text-tertiary">読み込み中...</span>
                ) : (
                  `リンクをクリックして${userCount.toLocaleString()}人の学生クリエイターと一緒に創作活動に参加しよう！`
                )}
              </p>
